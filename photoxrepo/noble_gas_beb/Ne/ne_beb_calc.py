#!/usr/bin/env python3
"""
Calculate BEB ionization cross sections for Ne atom
"""
...
def parse_ne_gaussian_improved(calc, filename):
    """
    ...
    if match:
        ...
        print(f"\n# Improved parsing results for Ne:")
...
def main():
    ...
    output_dir = current_dir.parent / 'results' / 'Ne'
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Initialize calculator
    calc = BEBCalculator('Ne')
    
    # Parse Gaussian output (if available)
    gaussian_file = current_dir / 'input.log'  # Ne uses input.log instead of ne_beb.log
    if not gaussian_file.exists():
        gaussian_file = current_dir / 'ne_beb.log'  # Try alternative name
    
    if not gaussian_file.exists():
        # Create sample data for Ne (literature values)
        print("# Warning: No Gaussian output found. Using literature values for Ne.")
        # Ne orbitals: 1s, 2s, 2p
        # Binding energies (eV): 1s: ~870, 2s: ~48, 2p: ~21.6
        # Kinetic energies (eV): 1s: ~1255, 2s: ~65, 2p: ~25
        calc.E_orb = [870/27.2114, 48/27.2114, 21.6/27.2114, 21.6/27.2114, 21.6/27.2114]
        calc.Ekin_orb = [1255/27.2114, 65/27.2114, 25/27.2114, 25/27.2114, 25/27.2114]
        calc.occ = [2, 2, 2, 2, 2]  # 1s2 2s2 2p6
    else:
        ...
        try:
            ...
            if not parse_ne_gaussian_improved(calc, gaussian_file):
    ...
    exp_data = get_experimental_data('Ne')
    if exp_data:
