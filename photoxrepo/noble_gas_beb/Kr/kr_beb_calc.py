#!/usr/bin/env python3
"""
Calculate BEB ionization cross sections for Kr atom
"""
...
def parse_kr_gaussian_improved(calc, filename):
    """
    ...
    if match:
        ...
        print(f"\n# Improved parsing results for Kr:")
...
def main():
    ...
    output_dir = current_dir.parent / 'results' / 'Kr'
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Initialize calculator
    calc = BEBCalculator('Kr')
    
    # Parse Gaussian output (if available)
    gaussian_file = current_dir / 'kr_beb.log'
    if not gaussian_file.exists():
        # Use literature values for Kr
        print("# Warning: No Gaussian output found. Using literature values for Kr.")
        # Kr orbitals: 1s, 2s, 2p, 3s, 3p, 3d, 4s, 4p
        # Binding energies (eV) - approximate values
        calc.E_orb = [
            14326/27.2114,  # 1s
            1921/27.2114,   # 2s
            1730/27.2114, 1730/27.2114, 1730/27.2114,  # 2p
            292/27.2114,    # 3s
            222/27.2114, 222/27.2114, 222/27.2114,     # 3p
            93/27.2114, 93/27.2114, 93/27.2114, 93/27.2114, 93/27.2114,  # 3d
            27.5/27.2114,   # 4s
            14.0/27.2114, 14.0/27.2114, 14.0/27.2114   # 4p
        ]
        # Kinetic energies - estimated
        calc.Ekin_orb = [
            20600/27.2114,  # 1s
            2760/27.2114,   # 2s
            2300/27.2114, 2300/27.2114, 2300/27.2114,  # 2p
            420/27.2114,    # 3s
            300/27.2114, 300/27.2114, 300/27.2114,     # 3p
            110/27.2114, 110/27.2114, 110/27.2114, 110/27.2114, 110/27.2114,  # 3d
            32/27.2114,     # 4s
            15/27.2114, 15/27.2114, 15/27.2114         # 4p
        ]
        calc.occ = [2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2]  # 1s2 2s2 2p6 3s2 3p6 3d10 4s2 4p6
    else:
        ...
        try:
            ...
            if not parse_kr_gaussian_improved(calc, gaussian_file):
                ...
                calc.occ = [2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2]
    ...
    exp_data = get_experimental_data('Kr')
    if exp_data:
