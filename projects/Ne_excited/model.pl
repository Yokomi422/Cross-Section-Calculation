...
%model = ( 
  ...
  'molecule',    "Ne",               # Used only for directory and descriptions
  'atoms',        ["Ne"],             # All atoms must be specified
  ...
  'nactive',       10,                 # number of active target orbitals (used for SEP and CAS)
  ...
  'active_orbs',   [10,0,0,0,0,0,0,0], # which orbitals for each symmetry to use as active, 
  ...
  'MAS', [
    8, [8,8], 'active', # Neon has 8 electrons in valence orbitals (essentially CAS(8,8))
  # For neon atom, we use an active space with 8 electrons in 8 orbitals (2s and 2p)
  ],
  ...
  'l2_MAS', [
    8, [8,9], 'active', # Neon ground state plus single excitations (N+1 electrons)
  ...
  'ncasscf_states', {'singlet', [4,0,0,0,0,0,0,0],
  ...
  'ntarget_states', {'singlet', [4,0,0,0,0,0,0,0],  # number of target states to calculate in each irreducible representation (IR)
                     'triplet', [2,0,0,0,0,0,0,0]},
